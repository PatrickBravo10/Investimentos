...
if st.session_state["authentication_status"]:
    ...
    if menu == "📊 Investimentos":
        ...
        if not df_ed_inv.empty:
            ...
            with g1:
                ...
                fig_inv_pie.update_traces(textinfo='percent+label', texttemplate='<b>%{label}</b><br><b>%{percent:.1%}</b>', textfont_size=20)
    ...
    # ==========================================
        ...
        df_gastos = pd.read_csv(StringIO(gastos_raw), on_bad_lines='skip') if gastos_raw else pd.DataFrame()
    ...
    # ==========================================
    # ABA 3: DASHBOARD & INSIGHTS (FINAL AJUSTADO)
    # ==========================================
        ...
        if gastos_raw:
            ...
            # 1. EVOLUÇÃO MENSAL: DUAS BARRAS LADO A LADO
            ...
            with c_g1:
                # 2. GRÁFICO DE ROSCA (DONUT) - SEM CORTES E FONTE GRANDE
            ...
            if st.button("💡 GERAR NOVOS INSIGHTS"):
                ...
                pool = [f"✅ Taxa de Poupança: **{taxa:.1f}%**.", f"🚨 Dreno: **'{max_cat}'**.", f"💰 Média Mensal: **R$ {(tot_r/len(df['periodo'].unique())):,.2f}**."]
                random.shuffle(pool)
                st.info(pool[0])
                st.success(pool[1])
        else:
