import streamlit as st
import pandas as pd
import plotly.graph_objects as go
import streamlit_authenticator as stauth
import requests
import base64
import json
from io import StringIO

# --- CONFIGURAÇÃO DA PÁGINA ---
st.set_page_config(page_title="Gestor Financeiro PRO", layout="wide")

# --- FUNÇÃO PARA PEGAR O DÓLAR (MELHORADA) ---
@st.cache_data(ttl=3600) # Guarda a cotação por 1 hora para ser mais rápido
def get_dollar_rate():
    try:
        # API pública e gratuita (AwesomeAPI)
        url = "https://economia.awesomeapi.com.br/json/last/USD-BRL"
        response = requests.get(url, timeout=10)
        data = response.json()
        cotacao = float(data["USDBRL"]["bid"])
        atualizacao = data["USDBRL"]["create_date"]
        return cotacao, atualizacao
    except Exception as e:
        return 5.50, f"Cotação Manual (Erro: {e})"

# --- CONFIGURAÇÕES DO GITHUB (SECRETS) ---
GITHUB_TOKEN = st.secrets["GITHUB_TOKEN"]
REPO_NAME = st.secrets["REPO_NAME"]
HEADERS = {"Authorization": f"token {GITHUB_TOKEN}"}

# --- FUNÇÕES DE PERSISTÊNCIA ---
def get_git_file(file_path):
    url = f"https://api.github.com/repos/{REPO_NAME}/contents/{file_path}"
    res = requests.get(url, headers=HEADERS)
    if res.status_code == 200:
        content = res.json()
        decoded = base64.b64decode(content['content']).decode('utf-8')
        return decoded, content['sha']
    return None, None

def save_git_file(file_path, content_str, sha, message):
    url = f"https://api.github.com/repos/{REPO_NAME}/contents/{file_path}"
    encoded = base64.b64encode(content_str.encode('utf-8')).decode('utf-8')
    payload = {"message": message, "content": encoded, "sha": sha}
    return requests.put(url, headers=HEADERS, data=json.dumps(payload))

# --- LOGIN ---
names = ["Patrick Bravo"]
usernames = ["admin"]
passwords = ["12345"]

authenticator = stauth.Authenticate(
    {"usernames": {usernames[0]: {"name": names[0], "password": passwords[0]}}},
    "cookie_invest", "key_invest", cookie_expiry_days=30
)

authenticator.login(location="main")

if st.session_state["authentication_status"]:
    
    # 1. BUSCAR COTAÇÃO
    dolar_hoje, data_dolar = get_dollar_rate()

    # 2. CARREGAR DADOS
    csv_data, csv_sha = get_git_file("dados.csv")
    metas_csv_data, metas_sha = get_git_file("metas.csv")

    if csv_data:
        df_ativos = pd.read_csv(StringIO(csv_data))
        if "origem" not in df_ativos.columns:
            df_ativos.insert(0, "origem", "B3")
    else:
        df_ativos = pd.DataFrame(columns=["origem", "tipo", "nome", "valor_atual", "aporte_mensal", "juros_mensal"])

    if metas_csv_data:
        df_metas_carregado = pd.read_csv(StringIO(metas_csv_data))
        meta_inicial = float(df_metas_carregado["valor_meta"].iloc[0]) if not df_metas_carregado.empty else 100000.0
        tempo_inicial = int(df_metas_carregado["tempo_anos"].iloc[0]) if not df_metas_carregado.empty else 10
    else:
        meta_inicial, tempo_inicial = 100000.0, 10

    # --- INTERFACE ---
    st.title("📊 Gestor Financeiro Global")
    st.info(f"💵 **Dólar Agora:** R$ {dolar_hoje:.2f} | **Atualizado em:** {data_dolar}")

    st.subheader("📝 Sua Carteira de Ativos")
    st.caption("Para apagar: selecione a linha e aperte 'Delete' no teclado ou use a lixeira à direita.")

    # EDITOR DE DADOS CONFIGURADO PARA FLOAT (CENTAVOS)
    df_editado = st.data_editor(
        df_ativos,
        num_rows="dynamic",
        use_container_width=True,
        column_config={
            "valor_atual": st.column_config.NumberColumn("Valor Atual", format="%.2f", help="Use ponto ou vírgula conforme seu sistema"),
            "aporte_mensal": st.column_config.NumberColumn("Aporte Mensal", format="%.2f"),
            "juros_mensal": st.column_config.NumberColumn("Juros Mensal (%)", format="%.2f%%"),
        }
    )

    # Lógica de cálculo do Valor Efetivo (Conversão de Dólar)
    def calcular_efetivo(row):
        origem = str(row.get("origem", "B3")).strip().lower()
        # Garante que o valor é float antes de calcular
        valor = float(row["valor_atual"]) if pd.notnull(row["valor_atual"]) else 0.0
        if origem == "avenue":
            return valor * dolar_hoje
        return valor

    df_editado["valor_efetivo"] = df_editado.apply(calcular_efetivo, axis=1)

    # --- SIDEBAR ---
    with st.sidebar:
        st.header(f"Olá, {st.session_state['name']}")
        
        st.subheader("🎯 Metas Financeiras")
        valor_meta = st.number_input("Objetivo (R$)", min_value=1.0, value=meta_inicial, format="%.2f")
        tempo_anos = st.slider("Prazo (Anos)", 1, 50, value=tempo_inicial)
        
        st.markdown("---")
        if st.button("💾 SALVAR ALTERAÇÕES"):
            with st.spinner("Sincronizando com GitHub..."):
                colunas_salvar = ["origem", "tipo", "nome", "valor_atual", "aporte_mensal", "juros_mensal"]
                csv_str = df_editado[colunas_salvar].to_csv(index=False)
                res1 = save_git_file("dados.csv", csv_str, csv_sha, "Update ativos")
                
                df_metas_save = pd.DataFrame([{"valor_meta": valor_meta, "tempo_anos": tempo_anos}])
                metas_csv_str = df_metas_save.to_csv(index=False)
                res2 = save_git_file("metas.csv", metas_csv_str, metas_sha, "Update metas")
                
                if res1.status_code in [200, 201] and res2.status_code in [200, 201]:
                    st.success("Tudo salvo com sucesso!")
                    st.balloons()
                    st.rerun()
                else:
                    st.error("Erro ao salvar no GitHub. Verifique sua conexão.")

        authenticator.logout("Sair", "sidebar")

    # --- DASHBOARD ---
    if not df_editado.empty:
        # Garantir tipos numéricos para os gráficos
        df_editado["valor_efetivo"] = pd.to_numeric(df_editado["valor_efetivo"], errors='coerce').fillna(0)
        total_brl = df_editado["valor_efetivo"].sum()
        
        c1, c2, c3 = st.columns(3)
        c1.metric("Patrimônio Total", f"R$ {total_brl:,.2f}")
        
        # Projeção de Juros Compostos
        meses = tempo_anos * 12
        projecao = [0.0] * (meses + 1)
        for _, row in df_editado.iterrows():
            v_inicial = float(row["valor_efetivo"])
            aporte = float(row["aporte_mensal"])
            taxa = float(row["juros_mensal"]) / 100
            
            valor_acumulado = v_inicial
            for m in range(meses + 1):
                if m > 0:
                    valor_acumulado = (valor_acumulado * (1 + taxa)) + aporte
                projecao[m] += valor_acumulado

        c2.metric(f"Estimativa ({tempo_anos} anos)", f"R$ {projecao[-1]:,.2f}")
        progresso = (total_brl / valor_meta) * 100 if valor_meta > 0 else 0
        c3.metric("Meta Alcançada", f"{progresso:.1f}%")

        # GRÁFICOS
        st.markdown("---")
        g1, g2 = st.columns(2)
        
        with g1:
            st.write("### 📂 Distribuição por Categoria")
            df_tipo = df_editado.groupby("tipo")["valor_efetivo"].sum().reset_index()
            fig_tipo = go.Figure(data=[go.Pie(labels=df_tipo["tipo"], values=df_tipo["valor_tipo"], hole=.4)])
            st.plotly_chart(fig_tipo, use_container_width=True)
            
        with g2:
            st.write("### 💎 Top Ativos")
            fig_nome = go.Figure(data=[go.Pie(labels=df_editado["nome"], values=df_editado["valor_efetivo"], hole=.4)])
            st.plotly_chart(fig_nome, use_container_width=True)

        # GRÁFICO DE EVOLUÇÃO (O que tinha saído)
        st.write("### 📈 Projeção de Crescimento Patrimonial")
        fig_evol = go.Figure()
        fig_evol.add_trace(go.Scatter(
            y=projecao, 
            mode='lines', 
            fill='tozeroy', 
            line=dict(color='#00FF00', width=4),
            name="Patrimônio"
        ))
        fig_evol.add_hline(y=valor_meta, line_dash="dash", line_color="red", annotation_text="Sua Meta")
        fig_evol.update_layout(xaxis_title="Meses", yaxis_title="Valor em R$")
        st.plotly_chart(fig_evol, use_container_width=True)

elif st.session_state["authentication_status"] is False:
    st.error("Login incorreto")
