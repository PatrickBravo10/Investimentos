...
if st.session_state["authentication_status"]:
    ...
    # =========================================================================
    # ABA 1: INVESTIMENTOS (RESTAURADA COM TODA A INTELIGÊNCIA)
    # =========================================================================
    if menu == "📊 Investimentos":
        ...
        with st.expander("📝 Editar Minha Carteira", expanded=True):
            df_ed_inv = st.data_editor(df_inv, num_rows="dynamic", use_container_width=True, key="editor_inv_original",
                column_config={
        ...
        # CÁLCULO REAL: Converte Avenue usando o Dólar
        ...
        if not df_ed_inv.empty:
            ...
            with st.expander("💱 Racional da Conversão (BRL vs USD)", expanded=False):
            ...
            st.markdown("---")
            st.subheader("⚖️ Rebalanceamento Estratégico")
            ...
            for i, row in df_bal.iterrows():
                with cols_b[i % len(cols_b)]:
                    st.write(f"**{row['tipo']}**")
                    m_p = st.number_input(f"Meta %", 0.0, 100.0, 100.0/len(df_bal), key=f"rebal_{row['tipo']}")
    ...
    # =========================================================================
    # ABA 2: FLUXO DE CAIXA (MANUAL COM 2 BOTÕES)
    # =========================================================================
        ...
        with c3:
            ...
            if st.button("💾 SALVAR ALTERAÇÕES", key="btn_topo", use_container_width=True):
                salvar_g_manual(st.session_state.editor_caixa_original)
                st.rerun()
        ...
        m2.metric("Saído", f"R$ {saiu:,.2f}", delta_color="inverse")
        m3.metric("Saldo Real", f"R$ {entrou - saiu:,.2f}")

        df_ed_caixa = st.data_editor(df_mes, num_rows="dynamic", use_container_width=True, key="editor_caixa_original",
            column_config={
        ...
        if st.button("🔄 Replicar Recorrentes para o Mês Seguinte"):
            ...
            if not df_rec.empty:
                ...
                save_git_file("gastos.csv", df_f.to_csv(index=False), gastos_sha, f"Replicate")
                st.success(f"Recorrentes copiados para {p_m}!")
                st.rerun()

    # =========================================================================
    # ABA 3: DASHBOARD (FONTE 14 / FORMATO 1,5k / HÍBRIDO)
    # =========================================================================
        ...
        if gastos_raw:
            ...
            fig_h.update_traces(textfont_size=14, textposition='outside', cliponaxis=False)
            st.plotly_chart(fig_h, use_container_width=True)

            col_g1, col_g2 = st.columns(2)
            with col_g1:
                # 2. ROSCA (DONUT) - SEM CORTES
            ...
            fig_a.add_trace(go.Bar(x=df_rec_a['periodo'], y=df_rec_a['acumulado'], name="Acum. Receita", marker_color='rgba(0, 200, 5, 0.4)',
                                   text=[f"<b>{v/1000:,.1f}k</b>".replace('.', ',') for v in df_rec_a['acumulado']], textposition='auto'))
            ...
            if st.button("💡 GERAR NOVOS INSIGHTS"):
                ...
                pool = [f"✅ Taxa de Poupança: **{taxa:.1f}%**.", f"🚨 Dreno: **'{max_cat}'**.", f"💰 Faturamento Médio: **R$ {(tot_r/len(df['periodo'].unique())):,.2f}**."]
                random.shuffle(pool)
